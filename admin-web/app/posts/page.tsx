...
export default function PostsPage() {
  ...
  const { data: posts = [], setData: setPosts } = useCached<Post[]>(
    profile ? 'posts:list' : null,
    async () => {
  ...
  async function handleDelete(id: string) {
    ...
    if (target && profile && target.authorUid !== profile.uid) {
      await writeAdminLog(profile, 'delete_post', {
        postId: id,
        postTitle: target.title,
        postAuthorUid: target.authorUid,
        postAuthorName: target.authorName,
      });
    }
  }
