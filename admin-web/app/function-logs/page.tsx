      ...
      <main className="flex-1 p-4 md:p-8 md:ml-0">
        ...
        {logs.length === 0 ? (
          <div className="bg-white dark:bg-dark-card rounded-xl p-12 text-center">
            <p className="text-gray-400 text-lg">에러 로그가 없습니다</p>
            <p className="text-gray-500 text-sm mt-2">Cloud Functions에서 에러가 발생하면 여기에 표시됩니다</p>
          </div>
        ) : (
              ...
              <div key={log.id}
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <span className="text-orange-400 text-xs font-medium px-2 py-0.5 bg-orange-400/10 rounded">
                        {log.function}
                      </span>
                      {log.code && (
                ...
                {expanded === log.id && (
                  <div className="mt-3 space-y-2">
                    {log.stack && (
                      <pre className="p-3 bg-gray-100 dark:bg-dark-input rounded-lg text-xs text-gray-500 font-mono overflow-x-auto max-h-48 overflow-y-auto whitespace-pre-wrap">
                        {log.stack}
                      </pre>
                    )}
                    <div className="flex flex-wrap gap-2 text-xs text-gray-400">
                      {Object.entries(log)
                        ...
                        .map(([k, v]) => (
                          <span key={k} className="bg-gray-100 dark:bg-dark-input px-2 py-1 rounded">
                            {k}: {String(v)}
                          </span>
                        ))}
                    </div>
