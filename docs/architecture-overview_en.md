...
| `onUserUpdated` | `users/{uid}` onUpdate | push on approval / role / suspension change |
| `onChatMessageCreated` | `chats/{cid}/messages/{mid}` onCreate | push to recipient |
| `suspensionScheduler` | onSchedule (hourly) | delete `suspendedUntil` when expired → cascades `onUserUpdated` |
| `cleanupOldPosts` | onSchedule | TTL cleanup for stale reports/logs |
