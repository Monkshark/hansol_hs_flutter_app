...
실제 배포 운영 중 부딪힌 14건의 기술 과제와 해결 방법을 정리합니다. 각 사례는 *문제 → 해결 → 결과*의 흐름을 유지하고, 관련 파일 경로를 함께 남깁니다.
...
**관련 파일:** `lib/api/meal_api.dart` (추정), `test/meal_api_test.dart`
...
**관련 파일:** `functions/index.js` (`suspensionScheduler`)
