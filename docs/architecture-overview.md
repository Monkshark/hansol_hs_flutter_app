...
| `onUserUpdated` | `users/{uid}` onUpdate | 승인/역할/정지 변경 시 푸시 |
| `onChatMessageCreated` | `chats/{cid}/messages/{mid}` onCreate | 수신자에게 채팅 푸시 |
| `suspensionScheduler` | onSchedule (매시간) | `suspendedUntil <= now`인 유저 필드 삭제 → `onUserUpdated` 연쇄 |
| `cleanupOldPosts` | onSchedule | TTL 만료된 신고/로그 정리 |
