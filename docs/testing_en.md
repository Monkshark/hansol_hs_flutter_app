...
A 4-layer test setup (Unit + Provider + Widget + Golden + Firestore Rules), totalling **180 tests**.
...
| Provider | 17 | `ProviderContainer` verifies AsyncNotifier directly |
| Widget | 17 | `ProviderScope.overrides` + mock notifier |
| Golden | 5 | PostCard PNG snapshot comparison |
| Repository | 8 | GetIt-based mock injection demo |
| Firestore Rules | 34 | `@firebase/rules-unit-testing` + emulator |
| Integration | 10 | `integration_test/` (optional) |
| **Total** | **146 Flutter + 34 Rules = 180** | |
...
## Measured Times
- Flutter unit/widget/provider/golden: **~3s**, 131 tests, local
