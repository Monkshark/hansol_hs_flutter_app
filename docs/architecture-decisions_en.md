...
- **Related files**: `lib/network/secure_storage_service.dart` (expected), `test/secure_storage_service_test.dart`
...
- **Unit (89)**: models / utils / parsers. Zero external deps
- **Provider (17)**: `ProviderContainer` verifies AsyncNotifier without a widget tree
- **Widget (17)**: `ProviderScope.overrides` injects mock notifiers; loading/error/empty branches
- **Golden (5)**: PostCard snapshot PNG comparison (`fake_cloud_firestore` + tolerance comparator)
...
- **Totals**: 146 (Flutter) + 34 (Rules) = **180**. Full breakdown in [testing_en.md](./testing_en.md)
