...
4계층 테스트 체계 (Unit + Provider + Widget + Golden + Firestore Rules)로 총 **558개** 테스트를 유지합니다.
...
| `testWidgets()` 위젯 | 80 | `ProviderScope.overrides` + Mock Notifier 기반 UI 검증 |
| Integration | 4 | `integration_test/` — 앱 네비게이션 E2E |
| Firestore Rules | 34 | `@firebase/rules-unit-testing` + 에뮬레이터 |
| **합계** | **524 Flutter + 34 Rules = 558** | |

> 측정: `grep -rE "^\s*test\(" test/` = 440, `grep -rE "^\s*testWidgets\(" test/` = 80, `integration_test/` 4개.
...
## 실행 시간 (실측)
- Flutter 전체 (`flutter test`): 520개 (`test` + `testWidgets`), 로컬 머신 기준 수 초~수십 초
